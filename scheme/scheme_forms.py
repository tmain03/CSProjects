...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        sum = scheme_eval(expressions.rest.first, env)
        env.define(signature, sum)
        return signature
        # END PROBLEM 4
        ...
        new_form = do_lambda_form(Pair(formals, body), env)
        env.define(symbol, new_form)
        return symbol
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        while expressions is not nil and is_scheme_true(value):
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        ...
        first = scheme_eval(expressions.first, env) # <- this is the first operation in the statement
        if is_scheme_false(first):
            return do_or_form(expressions.rest, env)
        else:  
            return first
    # for some reason this do-or-form wouldn't work when i formatted it like the and func 
    # END PROBLEM 12
