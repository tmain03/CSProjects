...
public class Model {
    ...
    public void tilt(Side side) {
        ...
        for (int c = 0; c < board.size(); c++) {
            int highest_merged = board.size() - 1;
            for (int r = board.size() - 1; r >= 0; r--) {
                Tile t = board.tile(c, r);
                if (t != null) {
                    int drow = r;
                    while (drow < board.size() - 1 && board.tile(c, drow + 1) == null) {
                        drow += 1;
                    }
                    if (drow < board.size() - 1 && board.tile(c, drow + 1).value() == t.value() && drow < highest_merged) {
                        drow += 1;
                        highest_merged -= 1;
                        score += 2 * t.value();
                    }
